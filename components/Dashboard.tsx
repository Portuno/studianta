      ...
      <>
        <div className="h-full flex flex-col gap-4 sm:gap-6 pb-32 animate-fade-in px-3 sm:px-4 overflow-y-auto no-scrollbar safe-area-inset-bottom">
          ...
          <section className="flex-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 pb-12">
              {sortedModules.map((mod, index) => {
                ...
                return (
                  <div 
                    ...
                    className={`glass-card aspect-square rounded-[1.5rem] sm:rounded-[2rem] flex flex-col items-center justify-center p-3 sm:p-4 transition-all duration-300 active:scale-95 relative border-2 min-h-[120px] touch-manipulation ${isActive ? 'border-[#D4AF37]/30 shadow-md' : 'border-transparent opacity-60 grayscale-[0.4]'}`}
                  >
                    <div className={`w-14 h-14 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center mb-2 sm:mb-3 transition-colors ${isActive ? 'bg-[#E35B8F]/10 text-[#E35B8F]' : 'bg-white/40 text-[#8B5E75]'}`}>
                      {getIcon(mod.icon, "w-7 h-7 sm:w-6 sm:h-6")}
                    </div>
                    <h4 className="font-cinzel text-[11px] sm:text-[10px] font-black text-[#4A233E] text-center uppercase tracking-widest px-1 leading-tight">
                      {mod.name}
                    </h4>
                    {!isActive && (
