...
const successUrl = Deno.env.get("STRIPE_SUCCESS_URL") || `${appUrl}/billing?status=success`;
...
serve(async (req: Request) => {
  try {
    ...
    if (!priceId) return new Response(JSON.stringify({ error: "Plan price not configured" }), { status: 500 });
