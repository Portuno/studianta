 ...
 */
export const parseOracleText = (text: string, isNightMode: boolean = false): JSX.Element[] => {
    ...
    if (currentParagraph.length > 0) {
      ...
      if (paragraphText) {
        elements.push(
          <p 
            key={`p-${keyCounter++}`} 
            className={`font-garamond text-base md:text-lg leading-relaxed mb-4 text-justify transition-colors duration-500 ${
              isNightMode ? 'text-[#E0E1DD]' : 'text-[#374151]'
            }`}
          >
    ...
    for (const match of allMatches) {
      ...
      if (match.type === 'bold') {
        ...
        const color = isNegative 
          ? (isNightMode ? 'text-rose-400' : 'text-[#E35B8F]')
  ...
  const CrownIcon = () => (
    <svg className={`w-5 h-5 inline-block mr-2 flex-shrink-0 transition-colors duration-500 ${
      isNightMode ? 'text-[#A68A56]' : 'text-[#D4AF37]'
  ...
  const OrnamentalDivider = () => (
    <div className="flex items-center justify-center my-6">
      ...
      <span className={`mx-3 text-sm transition-colors duration-500 ${
        isNightMode ? 'text-[#A68A56]' : 'text-[#D4AF37]'
      }`}>✦</span>
      <span className={`text-xl transition-colors duration-500 ${
        isNightMode ? 'text-[#A68A56]' : 'text-[#D4AF37]'
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isDictamenHeader && !inDictamenSection) {
      ...
      elements.push(
        <div key={`dictamen-start-${keyCounter++}`} className="mt-8 mb-4">
          <div className={`p-6 rounded-xl border-2 shadow-lg transition-colors duration-500 ${
            isNightMode 
              ? 'bg-gradient-to-br from-[rgba(48,43,79,0.8)] to-[rgba(48,43,79,0.6)] border-[#A68A56]/50' 
              : 'bg-gradient-to-br from-[#FFF9FB] to-[#FDEEF4] border-[#D4AF37]/50'
          }`}>
            <h3 className={`font-cinzel text-xl md:text-2xl font-bold uppercase mb-0 flex items-center transition-colors duration-500 ${
      ...
      if (line.startsWith('###')) {
        ...
        elements.push(
          <p 
            ...
            className={`font-garamond text-base md:text-lg leading-relaxed mb-3 text-justify pl-2 border-l-2 transition-colors duration-500 ${
              isNightMode 
                ? 'text-[#E0E1DD] border-[#A68A56]/30' 
                : 'text-[#374151] border-[#D4AF37]/30'
            }`}
          >
            {processedLine}
          </p>
    ...
    if (line.startsWith('###')) {
      ...
      elements.push(
        <h3 
          key={`h3-${keyCounter++}`} 
          className={`font-cinzel text-lg md:text-xl font-bold uppercase mt-8 mb-4 flex items-center transition-colors duration-500 ${
            isNightMode ? 'text-[#E0E1DD]' : 'text-[#4A233E]'
          }`}
        >
          <CrownIcon />
    ...
    if (line.match(/^[\*\-\•]\s+/)) {
      ...
      elements.push(
        <div key={`list-${keyCounter++}`} className="flex items-start mb-3">
          <span className={`font-bold mr-3 mt-1 transition-colors duration-500 ${
            isNightMode ? 'text-[#A68A56]' : 'text-[#D4AF37]'
          }`}>◆</span>
          <p className={`font-garamond text-base md:text-lg leading-relaxed flex-1 text-justify transition-colors duration-500 ${
            isNightMode ? 'text-[#E0E1DD]' : 'text-[#374151]'
          }`}>
